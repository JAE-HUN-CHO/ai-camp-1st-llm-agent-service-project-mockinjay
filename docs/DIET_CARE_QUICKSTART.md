...
- Ollama running locally with `qwen3.6:27b-mlx` and `nomic-embed-text-v2-moe`
...
export OLLAMA_MODEL="qwen3.6:27b-mlx"
...
curl -X POST "http://localhost:8000/api/diet-care/session/create" \
  -H "Authorization: Bearer ${CAREGUIDE_JWT_TOKEN:?set CAREGUIDE_JWT_TOKEN}"
```
...
### Issue: "Ollama is unavailable"

**Solution:** Start Ollama and verify the local models:
```bash
ollama serve
ollama list
```
...
```bash
# 1. Login
AUTH=$(curl -s -X POST "http://localhost:8000/api/auth/login" \
  -H "Content-Type: application/json" \
...
SESSION=$(curl -s -X POST "http://localhost:8000/api/diet-care/session/create" \
  -H "Authorization: Bearer $AUTH" | jq -r '.session_id')
