# Phase 0–3B ATAM-lite risk and traceability matrix

**Scope:** CareGuide Phase 0–3B traceability
**Decision boundary:** ADR-013 is Accepted; Phase 3A Health Records and Phase 3B Health Profile are separately approved. Phase 3C and later phases remain unauthorized.
...
| R-03 cross-user room/session/health mutation | JWT actor supplies another actor's resource ID or tries to cross profile ownership | reject before model/DB mutation; cross-user cases 100%; unauthorized write 0 | `test_actor_context.py`, Health Records and Health Profile fake/Mongo/HTTP owner tests | `unit.junit.xml`, `integration.junit.xml`, `http/health-*.json` | chat/health; authorization over permissive recovery |
| R-04 token/chat/health PII escapes | Canary appears in storage, console, application log or evidence writer | zero canary occurrences in all four sinks | logging canary + Health Records/Profile synthetic canary + artifact validator | `pii/canary.json`, selector/HTTP evidence, frontend test output | platform; observability loses raw payloads |
...
Every abuse case applicable to the approved Phase 0–3B scope is fail-closed in its
