...
def ocr_file(file_path, api_key=None):
    ...
    """
    # External OCR providers are intentionally disabled. Keep this seam so a
    # local OCR implementation can be added without reintroducing API keys.
    print(f"    OCR skipped for {file_path}: local OCR adapter is not configured")
    return False, None
