"""
Local Ollama/MongoDB vector embedding script for welfare programs and hospital data.

This script loads documents from MongoDB, generates embeddings using
OptimizedVectorDBManager and prepares dedicated namespaces:
...
async def embed_collection(
    ...
    """Embed a MongoDB collection with local Ollama embeddings."""
...
async def main(selected_collections: List[str]):
    ...
    print("=" * 80)
    print("🔮 Embedding welfare/hospital datasets with local Ollama")
    print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Embed welfare/hospital data with local Ollama.")
    parser.add_argument(
        "--collections",
        nargs="+",
