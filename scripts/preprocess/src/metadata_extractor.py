...
class MetadataExtractor:
    ...
    Uses the repository's local Ollama adapter
    """

    def __init__(self, model_name: str = "ollama", rate_limit: int = 10):
        """
        Initialize with the local Ollama model

        Args:
            model_name: retained for caller compatibility
            rate_limit: requests per second limit
        """
        self.client = OllamaSyncClient(model=os.getenv("OLLAMA_MODEL", "qwen3.6:27b-mlx"))
    ...
    def _retry_with_ollama(self, prompt: str, system_prompt: str) -> Optional[Dict]:
        """
        Retry the local Ollama request after a transient provider failure
        ...
        """
        try:
            self.stats["fallback_to_ollama"] += 1
            logger.info("Retrying local Ollama API...")
            ...
            logger.error(f"Ollama retry failed: {e}")
            return None
    ...
    def print_statistics(self):
        ...
        print(f"Ollama retries: {self.stats['fallback_to_ollama']}")
        print("="*60)
