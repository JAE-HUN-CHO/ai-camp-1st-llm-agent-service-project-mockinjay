...
export const ModalContainer: React.FC<ModalContainerProps> = ({
  ...
  useEffect(() => {
    ...
    const contentRoot = modalRef.current?.querySelector('[data-modal-content]') ?? modalRef.current;
    const focusableElements = modalRef.current?.querySelectorAll(
      'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
    );
    const initialFocusableElements = contentRoot?.querySelectorAll(
      'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
    );

    if (initialFocusableElements && initialFocusableElements.length > 0) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content */}
        <div data-modal-content className="flex-1 overflow-y-auto px-6 py-4">
          {children}
        </div>
