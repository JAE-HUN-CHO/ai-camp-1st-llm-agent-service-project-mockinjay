...
import { CKDStage, MealType, AnalysisStatus } from './diet-care';
import type {
  ActivityLevel,
  NutrientStatus,
  FoodItem,
...
export function isNutritionSuccess(value: unknown): value is NutritionAnalysisSuccess {
  return typeof value === 'object' && value !== null && (value as { type?: unknown }).type === 'success';
}

export function isNutritionError(value: unknown): value is NutritionAnalysisError {
  return typeof value === 'object' && value !== null && (value as { type?: unknown }).type === 'error';
}
...
/** Backward-compatible name used by the original diet-care UI. */
export const isImageFile = isValidImageFile;
...
export function isValidFileSize(file: File, maxSizeMB: number = 10): boolean {
  // The legacy caller passed raw bytes while the current API documents MB.
  // Preserve both forms during the frontend consolidation window.
  const maxSizeBytes = maxSizeMB > 1024 ? maxSizeMB : maxSizeMB * 1024 * 1024;
...
export const isValidNutrientValue = isValidNumericValue;

export function isValidNutrientData(value: unknown): value is NutrientData {
  if (typeof value !== 'object' || value === null) return false;
  const data = value as Record<string, unknown>;
  const required = ['calories', 'protein', 'carbohydrates', 'fat', 'sodium', 'potassium', 'phosphorus'];
  return required.every((key) => typeof data[key] === 'number' && isValidNumericValue(data[key] as number))
    && ['fiber', 'sugar'].every((key) => !(key in data) || (typeof data[key] === 'number' && isValidNumericValue(data[key] as number)));
}
...
export function isCKDStage(value: unknown): value is CKDStage {
  return typeof value === 'string' && Object.values(CKDStage).includes(value as CKDStage);
}
...
export function isValidFoodItem(item: unknown): item is FoodItem {
  ...
  // Legacy shape: { id, name, servingSize, nutrition: NutrientData }.
  if ('id' in food || 'servingSize' in food || 'nutrition' in food) {
    return typeof food.id === 'string' && food.id.length > 0
      && typeof food.name === 'string' && food.name.length > 0
      && typeof food.servingSize === 'string' && food.servingSize.length > 0
      && isValidNutrientData(food.nutrition);
  }
