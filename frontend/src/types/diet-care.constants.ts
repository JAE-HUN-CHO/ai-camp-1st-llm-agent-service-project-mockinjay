...
import { CKDStage, MealType } from './diet-care';
...
// Compatibility aliases for the original diet-care contract. Keep these
// derived from the canonical constants above so the two surfaces cannot drift.
export const MEAL_TYPE_INFO = Object.fromEntries(
  Object.entries(MEAL_TYPE_CONFIG).map(([mealType, info]) => [mealType, info])
) as typeof MEAL_TYPE_CONFIG;

export const IMAGE_FILE_CONSTRAINTS = {
  maxSizeBytes: FILE_UPLOAD_CONFIG.MAX_SIZE_BYTES,
  acceptedTypes: FILE_UPLOAD_CONFIG.ACCEPTED_TYPES,
} as const;

export const NUTRIENT_LIMITS = {
  [CKDStage.Stage1]: { protein: { max: 65 } },
  [CKDStage.Stage2]: { protein: { max: 60 } },
  [CKDStage.Stage3a]: { protein: { max: 50 } },
  [CKDStage.Stage3b]: { protein: { max: 45 } },
  [CKDStage.Stage4]: { protein: { max: 40 } },
  [CKDStage.Stage5]: { protein: { max: 35 } },
} as const;

export const CKD_STAGE_INFO = {
  [CKDStage.Stage1]: { ...CKD_STAGE_CONFIG[1] },
  [CKDStage.Stage2]: { ...CKD_STAGE_CONFIG[2] },
  [CKDStage.Stage3a]: { ...CKD_STAGE_CONFIG[3] },
  [CKDStage.Stage3b]: { ...CKD_STAGE_CONFIG[3] },
  [CKDStage.Stage4]: { ...CKD_STAGE_CONFIG[4] },
  [CKDStage.Stage5]: { ...CKD_STAGE_CONFIG[5] },
} as const;
