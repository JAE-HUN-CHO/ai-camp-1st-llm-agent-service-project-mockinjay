...
import api, { checkEmailDuplicate, checkNicknameDuplicate, type TermsData } from '../services/api';
...
const SignupPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTerms = async () => {
      // Keep the fallback usable in isolated UI tests and during backend
      // startup; the shared client remains the single transport seam.
      const terms = typeof api.get === 'function'
        ? (await api.get<{ terms?: TermsData }>('/api/terms/all')).data.terms ?? null
        : null;
      if (terms) {
        setTermsData(terms);
      } else {
